...
(define (download-uri-to-file uri file)
  ...
  (system (string-append "curl --location " uri " --output " file)))
...
(define (main cmd)
  (if (> (length cmd) 2)
      (parameterize ((*tmp-directory* (mkdtemp "/tmp/update-helper.XXXXXX"))
                     (*package-db-entry* (package-db-entry (second cmd))))
        (dynamic-wind
          (const #t)
          (lambda ()
            (let* ((pkg-name (second cmd))
                   (guix-archs (fourth (*package-db-entry*)))
          ...
          (lambda ()
            (delete-file-recursively (*tmp-directory*)))))
